...
public class Article2Fragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        if (view == null) {
            view = inflater.inflate(R.layout.fragment_article_2, container, false);
            recyclerViewinit();
            for (int p = 1; p < 4; p++) {
                resourceModel.getResultList("article", p, sessionid, listener);
            }
        }
        return view;
    }
    ...
    private void loadMoreData() {
        page += 1;
        if (page < 5) {
            resourceModel.getResultList("article", page, sessionid, listener);
        }
